...
function streamUrl(songId, kind) {
    return KTV.playback.streamUrl(songId, kind);
}
...
getSingList = async () => {
    const res = await KTV.queue.pending();
    singsList = res.data;
    return singsList;
};
...
send_message = (code, data) => {
    KTV.events.send(code, data).catch(function (err) {
        if (err.code !== 0) $.Toast(err.msg, 'error');
    });
};
...
setSinged = (flag = false) => {
    ...
    KTV.playback.markFinished(singsList[0].id).catch(function (err) {
        if (err.code !== 0) $.Toast(err.msg, "error");
    });
};
...
setSinging = (flag = false) => {
    if (singsList.length < 1) return;
    KTV.playback.markSinging(singsList[0].id).catch(function (err) {
        if (err.code !== 0) $.Toast(err.msg, "error");
    });
};
...
async function fetchPlaybackProfile(songId) {
    const res = await KTV.playback.profile(songId);
    return res.data;
}
...
window.onload = function() {
    ...
    KTV.events.on(2, function () { reSing(); });
    KTV.events.on(3, function () { nextSong(); });
    ...
    KTV.events.on(7, function (message) {
