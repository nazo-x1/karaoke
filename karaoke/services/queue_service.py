...
class QueueService:
    ...
    async def enqueue(self, song_id: int) -> ApiResult:
        return await run_guarded(
            '点歌失败',
            lambda: self._enqueue(song_id),
            not_found_msg='歌曲不存在',
        )

    async def _enqueue(self, song_id: int) -> ApiResult:
    ...
    async def set_top(self, song_id: int) -> ApiResult:
        async def action():
            ...
            await event_bus.publish_queue_changed()
            return ApiResult(msg=f"{history.name} 置顶成功")

        return await run_guarded('置顶失败', action, not_found_msg='歌曲不在队列中')
    ...
    async def remove(self, song_id: int) -> ApiResult:
        async def action():
            ...
            await self._histories.delete(history)
            await event_bus.publish_queue_changed()
            return ApiResult(msg=f"{history.name} 播放记录删除成功")

        return await run_guarded('移除队列项失败', action, not_found_msg='歌曲不在队列中')
